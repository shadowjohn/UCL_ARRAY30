...
# 不使用肥列的 app
# 2021-03-19 2077 也不能使用肥列
# 2021-07-03 vncviewer.exe 不需要肥列
...
message = ("\nUCLARRAY30 肥列輸入法\nBy 羽山秋人(https://3wa.tw)\nVersion: %s\n\n若要使用 Debug 模式：uclarray30.exe -d\n" % (VERSION));

def about_uclarray30():
...
#  os.mkdir("C:\\temp")
check_file_run = open(PWD + '\\uclarray30.lock', "a+")
try:  
...
#2018-07-13 1.12版增加
#檢查 C:\temp\uclarray30.ini 初始化設定檔
#取螢幕大小

#2019-03-02 調整，將 uclarray30.ini 跟隨在 uclarray30.exe 旁
INI_CONFIG_FILE = 'C:\\temp\\uclarray30.ini'
if my.is_file(INI_CONFIG_FILE):
  my.copy(INI_CONFIG_FILE,PWD+"\\uclarray30.ini")
  my.unlink(INI_CONFIG_FILE)
INI_CONFIG_FILE = PWD + "\\uclarray30.ini" 
...
debug_print("uclarray30.ini SETTING:")
...
is_all_fault = False
#my.unlink("array30.json")
#my.unlink("array30.cin")
if my.is_file(PWD + "\\array30.json") == False:
  if my.is_file(PWD + "\\array30.cin") == False:          
    is_all_fault=True   
  else:
...
if is_all_fault == True:
  ...
  message.set_markup("無字根檔，請自行下載行列「array30.cin」與uclarray30.exe放在一起執行")  
...
if is_need_trans_cin==True:
  ...
  cinapp.run( "array30" , "array30.cin",False)
...
if my.is_file(PWD + "\\array30.json") == False:
  message = gtk.MessageDialog(type=gtk.MESSAGE_ERROR, buttons=gtk.BUTTONS_OK)
  message.set_markup("缺少 array30.json")  
  response = message.run()
  #debug_print(gtk.ResponseType.BUTTONS_OK)
  if response == -5 or response == -4:
    ctypes.windll.user32.PostQuitMessage(0)
...
uclcode = my.json_decode(my.file_get_contents(PWD + "\\array30.json"))
...
def toAlphaOrNonAlpha():
  ...
  global user32 
  global win
  #2019-10-22 check screen size and uclarray30 position
  # 偵測肥列的位置，超出螢幕時，彈回
  #screen_width=user32.GetSystemMetrics(0)
  #screen_height=user32.GetSystemMetrics(1)  
  screen_width = gtk.gdk.screen_width()
  ...
  #2021-07-27 改成偵測現在肥列離哪個螢幕中心點比較近，如果超過該螢幕限範圍回，要修正位置
...
class TrayIcon():
    ...
    def reload_tray(self):
      ...
      if self.systray=="":
        ...
        self.systray = SysTrayIcon(ICON_PATH, "肥列輸入法：%s" % (VERSION) , menu_options) #, on_quit=self.m_quit)        
        self.systray.start()
      else:        
